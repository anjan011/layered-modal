<?php
    ...
    class SideBar extends ParameterClass {
        ...
        public function prepareParams() {
            ...
            $this->_params['accordionId'] = $this->_params['id'].'-accordion';

            #endregion

            #region [Offcanvas ID]

            $this->_params['offCanvasId'] = $this->_params['id'] . '-offcanvas';
...
HTML;
        ...
        public function urlMatches(array $link = []) : bool {

            $currentUrl = $this->getCurrentUrl();

            $url = ArrayValue::asString($link,'url','');
            $is_home = ArrayValue::asInt($link,'is_home',0) > 0;

            if($is_home) {
                return parse_url($currentUrl, PHP_URL_PATH) == $url;
            }
            ...
            if(!$url || $url == '#') {
                ...
                if(stripos($currentUrl,$url) !== false) {
                    return true;
                }
            ...
            if(!empty($children)) {
                ...
                foreach ($children as $child) {
                    ...
                    $url = ArrayValue::asString($child, 'url', '');

                    if (($url != '' && $url != '#') && stripos($currentUrl, $url) !== false) {
                        return true;
                    }
        ...
        private function generateLinkMarkup(array $link = []): string {
            ...
            $urlMatches = $this->urlMatches($link);
...
HTML;
                ...
                <div class="accordion-item">
                    <h2 class="accordion-header">
                        <button class="accordion-button <?= !$urlMatches ? 'collapsed' : ''?>" type="button" data-bs-toggle="collapse"
                                data-bs-target="#<?= $menuId ?>">
                            <?= $this->generateIconMarkup($iconClass) ?> <span
                                    class="text-truncate"><?= $linkText ?></span>
                        </button>
                    </h2>
                    <div id="<?= $menuId ?>" class="accordion-collapse collapse <?= $urlMatches ? 'show' : ''?>" data-bs-parent="#<?= $accordionId ?>">
